...
function Sidebar({
  ...
  const { isMounted, isMobile, state, openMobile, setOpenMobile } =
    useSidebar();

  if (!isMounted) {
    // You can return null or a skeleton loader for a better UX
    if (collapsible === "none") return null; // Or match its default state
    return (
      <div
        className="group peer text-sidebar-foreground hidden md:block"
        data-state="expanded" // Render in a default state
      ></div>
    );
  }
...
function SidebarMenuSkeleton({
  ...
  // Calculate the random width only on the client, after the initial render
  React.useEffect(() => {
